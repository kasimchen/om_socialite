...
class GoogleProvider extends AbstractProvider implements ProviderInterface
{
     ...
     * {@inheritdoc}
     */
    protected function getUserByIdToken(AccessTokenInterface $token)
    {
        $response = $this->getHttpClient()->get('https://www.googleapis.com/oauth2/v3/tokeninfo?id_token='.$token->getToken());
    ...
    protected function mapUserToObject(array $user)
    {
        return new User([
            ...
            'firstname' => $this->arrayItem($user, 'name.givenName'),
            'lastname' => $this->arrayItem($user, 'name.familyName'),
        ]);
    }
     ...
     * {@inheritdoc}
     */
    protected function mapUserToObjectIdToken(array $user)
    {
        return new User([
            'id' => $this->arrayItem($user, 'sub'),
            'username' => $this->arrayItem($user, 'email'),
            'nickname' => $this->arrayItem($user, 'nickname'),
            'name' => $this->arrayItem($user, 'name'),
            'email' => $this->arrayItem($user, 'email'),
            'avatar' => $this->arrayItem($user, 'picture'),
            'firstname' => $this->arrayItem($user, 'given_name'),
            'lastname' => $this->arrayItem($user, 'family_name'),
        ]);
    }
}
